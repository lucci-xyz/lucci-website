...
import { COLORS, SHADOWS } from '@/lib/constants/theme'
      ...
      <div className="bg-white w-full max-w-[100vw] overflow-x-hidden">
          ...
          <div className="py-6 sm:py-8">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-6 w-full">
              {TEAM_MEMBERS.map((member, index) => (
                <div
                  key={member.name}
                  className={`rounded-xl sm:rounded-2xl p-4 sm:p-6 md:p-8 bg-white hover:shadow-2xl hover:scale-105 hover:-translate-y-1 transition-all duration-300 ease-in-out w-full cursor-pointer animate-fade-in-up ${index === 1 ? 'delay-200' : ''}`}
        ...
        <Container>
          <div className="pb-6 sm:pb-8">
            <div className="rounded-xl sm:rounded-2xl p-4 sm:p-6 md:p-8 text-center w-full" style={{ backgroundColor: COLORS.backgroundLight, boxShadow: SHADOWS.lg }}>
              <h2 className="text-xl sm:text-2xl font-light mb-3 sm:mb-4" style={{ color: COLORS.primary }}>Get in Touch</h2>
              <p className="text-sm sm:text-base mb-4 sm:mb-6 font-light" style={{ color: COLORS.primary }}>
              Have questions or want to collaborate? Reach out to us.
            </p>
            <a
              href="mailto:contact@luccilabs.xyz"
              className="inline-flex items-center gap-2 text-sm sm:text-base hover:opacity-80 transition"
